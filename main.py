...
# Functions
def issuer_card(id_card):
    id_card = sd.askstring("Issuer Card ID", "What is the Issuer's Card ID?\t\t\t")

    if id_card:
        return id_card
    else:
        mb.showerror(
            "Issuer ID cannot be zero!",
            "Can't keep Issuer ID empty, it must have a value",
        )
...
def add_record():
    ...
    if bk_status.get() == "Issued":
        card_id.set(issuer_card(card_id))
    else:
        card_id.set("N/A")
    ...
    if surety:
        try:
            ...
            mb.showinfo(
                "Record added", "The new record was successfully added to your database"
            )
...
def change_availability():
    ...
    if BK_status == "Issued":
        surety = mb.askyesno(
            "Is return confirmed?", "Has the book been returned to you?"
        )
        if surety:
            cursor.execute(
        ...
        cursor.execute(
            "UPDATE Library SET bk_status=?, card_id=? where bk_id=?",
            ("Issued", issuer_card(card_id), BK_id),
        )
        connector.commit()
...
# Left Frame
Label(left_frame, text="Book Name", bg=lf_bg, font=lbl_font).place(x=98, y=15)
Entry(left_frame, width=25, font=entry_font, text=bk_name).place(x=45, y=45)

Label(left_frame, text="Book ID", bg=lf_bg, font=lbl_font).place(x=110, y=95)
bk_id_entry = Entry(left_frame, width=25, font=entry_font, text=bk_id)
bk_id_entry.place(x=45, y=125)

Label(left_frame, text="Author Name", bg=lf_bg, font=lbl_font).place(x=90, y=175)
Entry(left_frame, width=25, font=entry_font, text=author_name).place(x=45, y=205)

Label(left_frame, text="Status of the Book", bg=lf_bg, font=lbl_font).place(x=75, y=255)
...
# Right Bottom Frame
Label(
    RB_frame, text="BOOK INVENTORY", bg=rbf_bg, font=("Noto Sans CJK TC", 15, "bold")
).pack(side=TOP, fill=X)
